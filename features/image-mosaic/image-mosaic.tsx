...
export function ImageMosaic() {
  ...
  function handleDownload() {
    if (resultBlob) downloadBlob(resultBlob, `mosaic.${format === "png" ? "png" : "jpg"}`);
  }
    ...
    <div className="space-y-6">
      ...
      {!file ? (
        <FileDropzone
          ...
          onFiles={handleFiles}
          label="モザイクをかけたい画像をドラッグ&ドロップ、またはクリックしてアップロード"
        ...
        <div className="space-y-6">
          <div className="grid gap-5 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="mosaic-format">保存形式</Label>
              <Select items={formatLabels} value={format} onValueChange={(v) => v && setFormat(v as OutputFormat)}>
                <SelectTrigger id="mosaic-format" className="w-full">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <CanvasEditor effect="mosaic" file={file} outputFormat={format} jpgQuality={quality} onExport={handleExport} />

          {resultBlob && resultUrl && (
            <div ref={resultRef} className="space-y-4">
              <div className="flex flex-col items-center gap-3 rounded-2xl border border-border bg-card p-6">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={resultUrl} alt="モザイク加工後の画像" className="max-h-96 max-w-full rounded-lg" />
              ...
              <ResultShareActions shareText="【画像モザイク加工】写真にモザイクをかけました" title="画像モザイク加工" resultRef={resultRef} />
            </div>
          )}
